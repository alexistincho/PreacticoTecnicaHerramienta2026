...
if (!$es_validador) {
    $_SESSION["error"] = "No tenés permiso para enviar a corrección";
...
if (!$noticia || $noticia["estado"] != "Lista para Validación") {
    $_SESSION["error"] = "La noticia no puede enviarse a corrección";
    header("Location: ../view/validar_noticia.php");
    exit;
}

/*=========================CAMBIAR ESTADO A PARA CORRECCIÓN=========================*/

$sql_update = "UPDATE noticias SET estado = 'Para Corrección' WHERE id_noticia = ?";
...
$sql_historial = "INSERT INTO historial (id_noticia, id_usuario, estado_anterior, estado_nuevo) 
                  VALUES (?, ?, 'Lista para Validación', 'Para Corrección')";
...
$_SESSION["mensaje"] = "Noticia enviada a corrección";
header("Location: ../view/validar_noticia.php");
exit;